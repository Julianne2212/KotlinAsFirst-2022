...
fun deleteMarked(inputName: String, outputName: String) {
    ...
    for (i in File(inputName).readLines()) {
        if (i.isNotEmpty() && i[0] == '_') continue
        writer.write(i)
        writer.newLine()
    }
    writer.close()
...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val message = File(inputName).readText().lowercase()
    val array = mutableMapOf<String, Int>()
    for (string in substrings) {
        array[string] = counter(message, string.lowercase())
    }
    return array
}

fun counter(message: String, string: String): Int {
    var text = ""
    var array = 0
    for (i in message) {
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (i in input) {
        if (i.trim().length > length) length = i.trim().length
    }
    for (i in input) {
        var count = (length - i.trim().length) / 2
        while (count > 0) {
            output.write(" ")
            count = count - 1
        }
